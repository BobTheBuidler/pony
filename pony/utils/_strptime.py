...
from typing import Dict, Final, Iterable, Optional, Tuple, Type, TypeVar
...
class TimeRE(Dict[str, str]):
    ...
    def __init__(self, locale_time: Optional[LocaleTime] = None):
        ...
        super().__init__({
            ...
            'Z': self.__seqToRE((tz for tz_names in self.locale_time.timezone
                                ...
                                'Z'),
            '%': '%'})
        super().__setitem__('W', super().__getitem__('U').replace('U', 'W'))
    ...
    def __seqToRE(self, to_convert: Iterable[str], directive: str) -> str:
        ...
        for value in to_convert:
            ...
            return ''
        return f"(?P<{directive}>{'|'.join(map(re_escape, to_convert))})"

    def pattern(self, format: str) -> str:
...
def _calc_julian_from_V(iso_year: int, iso_week: int, iso_weekday: int) -> Tuple[int, int]:
...
def _strptime_time(data_string: str, format: str = "%a %b %d %H:%M:%S %Y") -> time.struct_time:
